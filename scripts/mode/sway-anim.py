...
class Workspace:
    ...
    async def add_win(self, id):
        new = Window(dict(x=0, y=0, width=100, height=100), id)
    ...
    async def eval_stack(self, i3):
        ...
        screen = self.screen
        cur = self.stack
        await cur.set(x=screen.x, y=screen.y, width=screen.width, height=screen.height)
    ...
    async def move_all(self, i3):
        ...
        for i in range(frames):
            ...
            cur = self.stack
            j = 0
            # fix this to get the final size, then animate only to that size
            while cur != None:
                if animid != aid:
                    return
...
class Window(Rect):
    ...
    async def move(self, i3, a, dx):
        ...
        await i3.command(
            f"[con_id={self.id}] resize set width {int(width)}px height {int(height)}px"
        )
        await i3.command(
            f"[con_id={self.id}] move absolute position {int(x)}px {int(y)}px"
        )
